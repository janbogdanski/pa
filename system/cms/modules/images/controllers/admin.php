...
class Admin extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model(array('images_m'));
		$this->lang->load(array('images', 'categories'));
...
//		}
	...
	public function index()
	{
		...
		$total_rows = $this->images_m->count_by($base_where);
		$pagination = create_pagination('admin/blog/index', $total_rows);

		// Using this data, get the relevant results
		$blog = $this->images_m->limit($pagination['limit'])->get_many_by($base_where);
	...
	public function create()
	{
		...
		if ($this->form_validation->run())
		{
			...
			if ($id)
			{
				$this->pyrocache->delete_all('images_m');
	...
	public function edit($id = 0)
	{
		...
		if ($this->form_validation->run())
		{
			...
			$result = $this->images_m->update($id, array(
				'title'				=> $this->input->post('title'),
				'slug'				=> $this->input->post('slug'),
				'category_id'		=> $this->input->post('category_id'),
				'keywords'			=> Keywords::process($this->input->post('keywords')),
	...
	public function publish($id = 0)
	{
		...
		if ( ! empty($ids))
		{
			...
			foreach ($ids as $id)
			{
				...
				if ($post = $this->images_m->get($id))
				{
					$this->images_m->publish($id);

					// Wipe cache for this model, the content has changed
					$this->pyrocache->delete('images_m');
		...
		if ( ! empty($post_titles))
		{
			...
			{
				$this->session->set_flashdata('success', sprintf($this->lang->line('images_mass_publish_success'), implode('", "', $post_titles)));
			}
		}
	...
	public function delete($id = 0)
	{
		...
		if ( ! empty($post_titles))
		{
			...
			{
				$this->session->set_flashdata('success', sprintf($this->lang->line('images_mass_delete_success'), implode('", "', $post_titles)));
			}
		}
	...
	public function _check_title($title, $id = null)
	{
		...
		return $this->images_m->check_exists('title', $title, $id);			
	}
	...
	public function _check_slug($slug, $id = null)
	{
		...
		return $this->images_m->check_exists('slug', $slug, $id);
	}
	...
	public function ajax_filter()
	{
		...
		$results = $this->images_m->search($post_data);
